(() => {
  ...
  const DYNASTIES = {
    ...
    chen: {
      ...
      subtitle: '選擇紀年，查看南陳州郡縣、地方長官、方鎮長官、封爵及同時期後梁、王琳政區。'
    }
  };
  ...
  const localOfficerLegend = $('localOfficerLegend');
  ...
  function qiaoNote(entity, phase, year) {
    ...
    if (!info) return null;
    const verifiedPage=currentDynasty.key==='southern-liang'
      ? window.LIANG_QIAO_SOURCES?.pages?.[String(info.qiao_table_page_index)] : null;
    const source={
      book_page:verifiedPage?.book_page || info.book_page || null,
      pdf_page:verifiedPage?.pdf_page || info.pdf_page || null,
      page_index:info.qiao_table_page_index ?? null,
      origin:info.origin || '',
      source_title:verifiedPage?.source_title||'',
      editorial_note:verifiedPage?'○為梁有陳無，△為梁無陳有，無標為梁陳共有；符號不表示全朝每年存續。此處已校合頁碼，同名侨置關係仍須逐條覆核。':''
    };
  ...
  function chenFiefInfo(match,year) {
    ...
    const familyColor=window.CHEN_PERSON_FORMATS?.fiefRecordStyle(match,year);
    return {
      title:record.kind==='prince'?'南陳宗室王國資料':'南陳開國爵資料',
      summary:`${phase.fief}國：${chenFiefText(match)}`,
      paragraphs:[
        familyColor?.predecessor?`封君顏色：${familyColor.source}`:'',
  ...
  function southernPrefectureDisplayName(item) {
    const name=String(item?.name||'');
    const hasAttachedFief=Boolean((item?.chenFiefs||[]).length||(item?.liangFiefs||[]).length);
    return hasAttachedFief ? name.replace(/郡$/,'國') : name;
  }

  function applySouthernPrefectureDisplayNames(states) {
    for(const state of states)for(const row of state.rows)row.displayName=southernPrefectureDisplayName(row);
  }
  ...
  function itemDisplaysAsKingdom(item) {
    return Boolean(item?.kingdom||(item?.displayName&&item.displayName!==item.name&&/國$/.test(item.displayName)));
  }

  // ---------- 南陳地方長官：本年曾任，不以十二月末仍在任為條件 ----------
  function localOfficerData() {
    return currentDynasty.key==='southern-liang' ? window.LIANG_LOCAL_OFFICIALS : window.CHEN_LOCAL_OFFICIALS;
  }

  function localOfficerRecords(year) {
    if(currentDynasty.key==='southern-liang')return (window.LIANG_LOCAL_OFFICIALS?.years?.[String(year)]?.local_officers||[])
      .filter(record=>Number(record.year)===Number(year)&&['confirmed','probable'].includes(record.annual_presence_status));
    const originals=window.CHEN_LOCAL_OFFICIALS?.years?.[String(year)]?.local_officers||[];
    let records=originals.filter((record)=>Number(record.year)===Number(year)&&['confirmed','probable'].includes(record.annual_presence_status));
    // The user identified these two 588 rows as one appointment to the dual
    // prefecture. Keep both source IDs; do not generalize to other years/names.
    if(Number(year)===588) {
      const pair=records.filter((record)=>['CS-0431','CS-0438'].includes(record.official_id));
      if(pair.length===2&&pair.every((record)=>record.person_id==='PER-CC650C22182B')) {
        const primary=pair.find((record)=>record.official_id==='CS-0431');
        const combined={...primary,place:'南琅邪、彭城二郡',office:'琅邪彭城二郡太守',
          display_correction:'依使用者校訂：588年陳祗的琅邪太守、彭城太守合為琅邪彭城二郡太守，列於揚州下。原始兩筆記錄及任期推定保留。',
          combined_source_ids:pair.map((record)=>({annual_presence_id:record.annual_presence_id,official_id:record.official_id,evidence_mapping_id:record.technical_trace?.evidence_mapping_id})),
          administrative_link:{...primary.administrative_link,target_level:'prefecture',link_status:'user_corrected_dual_prefecture',link_method:'explicit_user_correction',
            attach_to_snapshot:true,hierarchy_active_in_year:true,state_assignment_asserted:true,
            state_id:'ch_s0001',state_name:'揚州',state_order:1,prefecture_id:'ch_p0002',prefecture_name:'南琅邪、彭城二郡',prefecture_order:2,
            normalized_place_key:'南琅邪彭城二郡',active_candidate_count:1,global_candidate_count:1,candidate_ids:['ch_p0002']}};
        records=records.filter((record)=>!pair.includes(record)).concat(combined);
      }
    }
    return records.sort((left,right)=>Number(left.display_order)-Number(right.display_order));
  }

  function localOfficerValue(value) {
    if (Array.isArray(value)) return value.length?value.map(localOfficerValue).join('、'):'無';
    if (value&&typeof value==='object') return Object.entries(value).map(([key,item])=>`${key}=${localOfficerValue(item)}`).join('；');
    const result=String(value??'').trim();return result||'無';
  }

  function localOfficerTenure(record) {
    return localOfficerData()?.tenures_by_id?.[record.official_id]||{};
  }
  ...
  function localOfficerDetachedReason(record) {
    const link=record.administrative_link||{};
    if(link.attach_to_snapshot!==true)return `本年行政區劃快照沒有可唯一連接的${link.target_level==='county'?'縣':'郡'}級實體（${link.link_status||'未標連接狀態'}）；保留記錄但不按名稱猜配。`;
    if(link.hierarchy_active_in_year!==true||link.state_assignment_asserted!==true)return '本年行政區劃實體有效性或隸州關係未獲斷言；保留記錄但不掛入州郡縣層級。';
    return '指定的本年州、郡或縣穩定ID沒有唯一目標；保留記錄且未使用名稱回退。';
  }

  function sortLocalOfficers(records) {
    const orderValue=(record)=>{
      const value=record.intra_year_order;
      return value===null||value===undefined||value===''||!Number.isFinite(Number(value))
        ? Number.POSITIVE_INFINITY
        : Number(value);
    };
    records.sort((left,right)=>{
      // The generated display order is a topological order even when a person
      // has only a partial historical rank. Do not move a known last rank ahead
      // of unranked people whose relative position is still constrained.
      if(Number.isFinite(left.display_order)&&Number.isFinite(right.display_order))return left.display_order-right.display_order;
      const leftOrder=orderValue(left),rightOrder=orderValue(right);
      if(leftOrder!==rightOrder)return leftOrder-rightOrder;
      return Number(left.display_order)-Number(right.display_order);
    });
  }

  function attachLocalOfficers(states,year) {
    const detached=[];
    for(const state of states)for(const row of state.rows){row.localOfficers=[];for(const county of row.counties)county.localOfficers=[];}
    for(const record of localOfficerRecords(year)) {
      const link=record.administrative_link||{};
      if(link.attach_to_snapshot!==true||link.hierarchy_active_in_year!==true||link.state_assignment_asserted!==true) {
        detached.push({record,reason:localOfficerDetachedReason(record)});continue;
      }
      const targetGroup=currentDynasty.key==='southern-liang'?'liang':'chen';
      const stateTargets=states.filter((state)=>state.group===targetGroup&&state.id===link.state_id);
      const rowTargets=stateTargets.flatMap((state)=>state.rows.filter((row)=>row.id===link.prefecture_id));
      if(stateTargets.length!==1||rowTargets.length!==1) {
        detached.push({record,reason:localOfficerDetachedReason(record)});continue;
      }
      if(link.target_level==='county') {
        const countyTargets=rowTargets[0].counties.filter((county)=>county.id===link.county_id);
        if(countyTargets.length!==1){detached.push({record,reason:localOfficerDetachedReason(record)});continue;}
        countyTargets[0].localOfficers.push(record);
      } else if(link.target_level==='prefecture') {
        rowTargets[0].localOfficers.push(record);
      } else {
        detached.push({record,reason:'記錄未標示郡級或縣級連接目標；保留記錄且不猜配。'});
      }
    }
    for(const state of states)for(const row of state.rows){sortLocalOfficers(row.localOfficers);for(const county of row.counties)sortLocalOfficers(county.localOfficers);}
    return detached.sort((left,right)=>Number(left.record.display_order)-Number(right.record.display_order));
  }
  ...
  function mergeGovernorRecords(left,right) {
    if (!left) return {
      ...right,
  ...
  function appendGovernorExtra(extras,displayRecord,reason) {
    ...
    const reasons=[extras[index].reason,reason].filter(Boolean);
    extras[index]=mergeGovernorRecords(extras[index],incoming);
    extras[index].reason=[...new Set(reasons)].join('；');
  }
    ...
    return {
      ...
      paragraphs:[
        ...
        pageIndexes.length ? (currentDynasty.key==='southern-liang'
          ? `方鎮年表PDF第${pageIndexes.map(p=>Number(p)+1).join('、')}頁（OCR頁索引從0起：${pageIndexes.join('、')}）。`
          : `方鎮年表資料頁序：${pageIndexes.join('、')}。`) : '',
  ...
  function attachGovernors(states,year,targetGroup) {
    const extras=[];
    for (const state of states) {state.governor=null;state.governorIndex=null;}
  ...
  function buildLiangSnapshot(year) {
    ...
    for (const stateEntity of window.LIANG_DATA?.states||[]) {
      ...
      for (const originalPrefEntity of stateEntity.p||[]) {
        const prefEntity=window.LIANG_COUNTY_MODEL?.correctedPrefecture(originalPrefEntity)||originalPrefEntity;
        ...
        const row={
          ...
          source:pp.source||liangSource(prefName,pp),entity:prefEntity,phase:pp,directCounties:false,unknownPrefecture:false,
    ...
    states.sort((a,b)=>a.order-b.order);
    const reviewedCounties=window.LIANG_COUNTY_MODEL?.attachCounties(states,year);
    const virtualFiefs=attachLiangFiefs(states,year);
    applySouthernPrefectureDisplayNames(states);
    const extraGovernorStates=attachGovernors(states,year,'liang');
    const detachedLocalOfficers=attachLocalOfficers(states,year);
    return {states,virtualFiefs,extraGovernorStates,detachedLocalOfficers,pendingCountyAssignments:reviewedCounties?.pending||[]};
  }
  ...
  function buildChenSnapshot(year) {
    ...
    applySouthernPrefectureDisplayNames(chen);
    ...
    return {states,virtualFiefs,extraGovernorStates,detachedLocalOfficers};
  }
  ...
  function compareSnapshots(current,previous,year) {
    ...
    for (const state of current.states) {
      ...
      for (const row of state.rows) {
        ...
        const rowName=itemDisplayName(row)||'州直領縣',oldRowName=itemDisplayName(oldRow)||'州直領縣';
        if (!oldRow) { rowNotes.push(`${rowName}為本年新增、復置或改隸至本州`); rowSources.push(row.source); }
        else if (oldRow.name!==row.name) { rowNotes.push(`${oldRowName}改稱${rowName}`); rowSources.push(row.source,oldRow.source); }
    ...
    return {states:current.states,virtualFiefs:current.virtualFiefs,extraGovernorStates:current.extraGovernorStates||[],detachedLocalOfficers:current.detachedLocalOfficers||[],pendingCountyAssignments:current.pendingCountyAssignments||[],removedChanges};
  }
  ...
  function createChenColoredAuxButton(text,info,className='',year=null,people=[],fiefMatch=null) {
    const value=String(text||''),button=createAuxButton('',info,className);
    const familyStyle=fiefMatch&&window.CHEN_PERSON_FORMATS?.fiefRecordStyle(fiefMatch,year);
    const ownerStyles=familyStyle?[familyStyle]:people.map(person=>window.CHEN_PERSON_FORMATS?.fiefStyle(typeof person==='string'?person:person.person,year));
  ...
  function chenFiefPersonColor(match,year) {
    const familyStyle=window.CHEN_PERSON_FORMATS?.fiefRecordStyle(match,year);
    if(familyStyle?.color)return familyStyle.color;
  ...
  function createLocalOfficerButton(record,year,className,detachedReason='') {
    ...
    const appointmentOnly=currentDynasty.key==='southern-liang'&&record.tenure_status==='appointment_only';
    button.appendChild(document.createTextNode(`　${record.office}${appointmentOnly?'〔授官〕':''}${detachedReason?`（${record.place}）`:''}`));
    ...
    button.setAttribute('aria-label',`${name}，${record.office}，${detachedReason?`${record.place}，`:''}${appointmentOnly?'本年授官確認，實際履任未詳':`本年任職${record.annual_presence_status==='probable'?'推定，斜體':'確認，正常字體'}`}${detachedReason?'，行政區劃未連接':''}`);
    return button;
  }

  function renderLocalOfficerBox(records,year,labelText='地方長官') {
    if(!records?.length)return null;
    const box=document.createElement('section');box.className='local-officer-box';box.setAttribute('aria-label',`${currentDynasty.key==='southern-liang'?'本年見載':'本年曾任'}${labelText}`);
    const label=document.createElement('span');label.className='local-officer-label';label.textContent=labelText;box.appendChild(label);
    const entries=document.createElement('div');entries.className='local-officer-entries';
    for(const record of records) {
    ...
    if(records.length>1&&records.some((record)=>['unresolved','unknown'].includes(record.intra_year_order_status))){
      const note=document.createElement('small');note.className='local-officer-order-note';note.textContent='年內先後未詳';entries.appendChild(note);
    }
    box.appendChild(entries);return box;
  }

  function createGovernorIndexButton(record,year,index) {
    if (!record||!Number.isInteger(index)||index<1) return null;
    const button=document.createElement('button');
    button.type='button';button.className='governor-index-link';button.textContent=`{${index}}`;
    button.dataset.info=registerAuxInfo(governorInfo(record,year));
    button.title=`本年方鎮資料第${index}號；點擊查看完整考證`;
    button.setAttribute('aria-label',`${record.state}本年方鎮資料第${index}號：查看完整考證`);
    return button;
  }
  ...
  function stateMatches(state) {
    ...
    const text=[state.name,...(state.governor?.summary_lines||[]),...state.rows.flatMap((r)=>[r.name,itemDisplayName(r),...(r.localOfficers||[]).map(localOfficerSearchText),...itemFiefLabels(r),...r.counties.flatMap((c)=>[c.name,...(c.localOfficers||[]).map(localOfficerSearchText),...itemFiefLabels(c)])])].join(' ');
  ...
  function renderState(state,year) {
    ...
    const h2=document.createElement('h2'),stateName=makeNameSpan(state,'',state.governor?governorInfo(state.governor,year):null);
    ...
    for (const row of state.rows) {
      ...
      if (!row.counties.length) {
        ...
        for (const c of row.counties) {
          const group=document.createElement('div');group.className='county-officer-group';
          const item=makeNameSpan(c,'county-item');appendFiefDetails(item,c,'county',year);group.appendChild(item);
          const countyOfficers=renderLocalOfficerBox(c.localOfficers,year,'縣級長官');if(countyOfficers)group.appendChild(countyOfficers);
  ...
  function renderDetachedLocalOfficers(items,year) {
    const query=normalizeName(searchInput.value.trim());
    const visible=(items||[]).filter((item)=>!query||normalizeName(`${localOfficerSearchText(item.record)} ${item.reason}`).includes(query));
    if(!visible.length||stateSelect.value||changedOnly.checked)return null;
    const card=document.createElement('article');card.className='local-officer-detached-card';
    const h=document.createElement('h2');h.textContent='本年地方長官未連接記錄';card.appendChild(h);
    const note=document.createElement('p');note.className='local-officer-detached-note';note.textContent=currentDynasty.key==='southern-liang'
      ? '下列人物依《梁書》《陳書》有本年授官或在任記載，但本年沒有可唯一連接的州郡縣；原文、履任狀態及繫年依據仍可點閱。'
      : '下列人物依主表可確認或推定在本年曾任其職，但無法與本年行政區劃快照中的唯一州、郡或縣實體精確連接。記錄完整保留，不據名稱猜配。';card.appendChild(note);
    const list=document.createElement('div');list.className='local-officer-detached-list';
  ...
  function renderPendingLiangCounties(items,year) {
    const query=normalizeName(searchInput.value.trim()),selected=stateSelect.value;
    if(selected||changedOnly.checked)return null;
    const visible=(items||[]).filter(item=>!query||normalizeName([
      item.assignment.name,item.assignment.prefecture_name,item.assignment.state_name,item.source.excerpt
    ].join(' ')).includes(query));
    if(!visible.length)return null;
    const card=document.createElement('details');card.className='liang-pending-counties';card.open=!!query;
    const summary=document.createElement('summary');summary.textContent=`本年未掛接的縣條目（${visible.length}個時段）`;card.appendChild(summary);
    const note=document.createElement('p');note.textContent='這些縣條目保留了人工意見或待核時段，尚未唯一接入本年政區；列在此處不表示本年確實存在。';card.appendChild(note);
    for(const item of visible){
      const row=document.createElement('p');
      row.appendChild(createAuxButton(`${item.assignment.name} · ${item.assignment.prefecture_name||item.assignment.state_name||'歸屬未定'}`,{
        title:'縣郡歸屬與時段待核',summary:`${formatYearLabel(year)} · ${item.assignment.name}`,
        paragraphs:[item.reason],sources:[item.source]
      },'liang-pending-county-link'));
      const reason=document.createElement('span');reason.textContent=`　${item.reason}`;row.appendChild(reason);card.appendChild(row);
    }
    return card;
  }
  ...
  function renderResults(states,virtualFiefs,extraGovernorStates,detachedLocalOfficers,year,pendingCountyAssignments=[]) {
    ...
    if (currentDynasty.key==='southern-liang') {
      ...
      if (filtered.length) {
        ...
        const small=document.createElement('small');small.textContent='502—557年逐年州郡；縣依校勘時段隨郡列示';heading.appendChild(small);section.appendChild(heading);
  ...
  function appendSourceEntry(body,source,options={}) {
    ...
    h.textContent=source.source_label || (source.page_index!=null&&!source.book_page&&!source.pdf_page
  ...
  function appendLocalOfficerDetailSection(body,title,value,className='') {
    const section=document.createElement('section');section.className=['officer-detail-section',className].filter(Boolean).join(' ');
    const heading=document.createElement('h3');heading.textContent=title;section.appendChild(heading);
    const values=Array.isArray(value)?value:[value];
    for(const item of values.filter((entry)=>String(entry??'').trim())){
      const p=document.createElement('p');p.textContent=String(item);section.appendChild(p);
    }
    if(section.children.length===1){const p=document.createElement('p');p.className='empty';p.textContent='主表未另載。';section.appendChild(p);}
    body.appendChild(section);return section;
  }
  ...
  function renderLocalOfficerDetail(body,info) {
    ...
    if(currentDynasty.key==='southern-liang'&&tenure.source_citation)appendLocalOfficerDetailSection(body,'史料出處',tenure.source_citation);
    const annualLabel=currentDynasty.key==='southern-liang'?'confirmed（本年原典記載可確認；是否實際履任見下項）'
      :record.annual_presence_status==='confirmed'?'confirmed（本年任職可確認，正常字體）':'probable（本年任職為推定，斜體）';
    const boundaryLabels={confirmed:'confirmed（任期起止邊界已確認）',probable:'probable（任期邊界為推定）',unknown:'unknown（任期邊界未詳）'};
    appendLocalOfficerDetailSection(body,'確定性',[`本年任職：${annualLabel}`,`任期邊界：${boundaryLabels[record.tenure_boundary_status]||record.tenure_boundary_status||'unknown（未詳）'}`]);
    appendLocalOfficerDetailSection(body,'任職性質',[tenure.tenure_nature||'未另標',tenure.tenure_status?`履任狀態：${tenure.tenure_status}`:'']);

    const evidenceSection=document.createElement('section');evidenceSection.className='officer-detail-section';
    const evidenceHeading=document.createElement('h3');evidenceHeading.textContent='史料原文';evidenceSection.appendChild(evidenceHeading);
    const evidenceParts=String(tenure.evidence_text||'').split(/\n\s*\n/).map((part)=>part.trim()).filter(Boolean);
    if(evidenceParts.length)for(const part of evidenceParts){const quote=document.createElement('blockquote');quote.className='officer-source-quote';quote.textContent=part;evidenceSection.appendChild(quote);}
    else {const p=document.createElement('p');p.className='empty';p.textContent='主表未保存可展示的史料原文。';evidenceSection.appendChild(p);}
    body.appendChild(evidenceSection);

    const anchorSection=document.createElement('section');anchorSection.className='officer-detail-section';
    const anchorHeading=document.createElement('h3');anchorHeading.textContent='時間錨點';anchorSection.appendChild(anchorHeading);
    const anchors=tenure.time_anchors||[],basis=record.annual_presence_basis||record.reasoning_source||[];
    if(anchors.length||basis.length){
      const list=document.createElement('ul');list.className='officer-anchor-list';
      for(const anchor of anchors){const li=document.createElement('li');li.textContent=`${anchor.label||anchor.type||'時間錨點'}：${anchor.value||'未詳'}`;list.appendChild(li);}
      for(const reason of basis){const li=document.createElement('li');li.textContent=`本年投射依據｜${reason.label||reason.type||'既有結論'}：${reason.source_value||reason.source_field||'主表既有結構化欄位'}`;list.appendChild(li);}
      anchorSection.appendChild(list);
    } else {const p=document.createElement('p');p.className='empty';p.textContent='主表未另載時間錨點。';anchorSection.appendChild(p);}
    body.appendChild(anchorSection);

    const orderBasis=Array.isArray(record.intra_year_order_basis)
      ? record.intra_year_order_basis
      : [record.intra_year_order_basis];
    const orderNotes=[record.intra_year_order_note,...orderBasis.map((item)=>typeof item==='string'?item:item?.label)]
      .filter((item)=>typeof item==='string'&&item.trim())
      .map((item)=>item.trim())
      .filter((item,index,items)=>items.indexOf(item)===index);
    appendLocalOfficerDetailSection(body,'綜合考證',[record.display_correction,tenure.comprehensive_analysis,...(tenure.research_notes||[]),...orderNotes],'officer-research-note');
    const career=tenure.career_links||{},careerLines=[];
    if(career.career_chain_text)careerLines.push(career.career_chain_text);
    const careerLabels={previous_roles:'上一官',next_roles:'下一官',predecessor_candidates:'前任候選',successor_candidates:'繼任候選',unresolved_links:'未定關係'};
    for(const [key,label] of Object.entries(careerLabels))for(const link of career[key]||[])careerLines.push(`${label}：${link.related||'未標'}${link.order_basis?`（${link.order_basis}）`:''}${link.review_state==='待考據'?'〔待考據〕':''}`);
    appendLocalOfficerDetailSection(body,'前後任或職業鏈資訊',careerLines);

    const details=document.createElement('details');details.className='technical-trace';
    const summary=document.createElement('summary');summary.textContent='技術資訊 / 數據追溯';details.appendChild(summary);
    const dl=document.createElement('dl'),trace={...(tenure.technical_trace||{}),...(record.technical_trace||{})};
    const technicalEntries={
      annual_presence_id:record.annual_presence_id,
      official_id:record.official_id,
      person_id:record.person_id,
      ...(record.combined_source_ids?{combined_source_ids:record.combined_source_ids}:{}),
      year_end_decision_id:trace.year_end_decision_id||trace.legacy_year_end_decision_ids,
      evidence_id:trace.evidence_id||trace.evidence_ids,
      evidence_mapping_id:trace.evidence_mapping_id||trace.evidence_mapping_ids,
      source_timeline_id:trace.source_timeline_id,
      normalized_tenure_id:trace.normalized_tenure_id,
      tenure_group_id:trace.tenure_group_id,
      adjudication_id:trace.adjudication_id,
      audit_issue_id:trace.audit_issue_id||trace.audit_issue_ids,
      phase6_revision_ids:trace.phase6_revision_ids,
      change_log_ids:trace.change_log_ids,
      intra_year_order:record.intra_year_order,
      intra_year_order_status:record.intra_year_order_status,
      intra_year_order_basis:record.intra_year_order_basis,
      confidence:record.confidence||record.annual_presence_status,
      reasoning_source:record.reasoning_source||record.annual_presence_basis,
      ...
      ...(currentDynasty.key==='southern-liang'?{epub_trace:trace}:{}),
      master_sha256:record.projection_method==='user_year_specific_supplement'?'不適用：本條為使用者588年專項補訂，未寫回原Excel':localOfficerData()?.meta?.source_master_sha256
    };
    ...
    if(info.detachedReason){const p=document.createElement('p');p.className='technical-detached-reason';p.textContent=`行政快照連接：${info.detachedReason}`;details.appendChild(p);}
    body.appendChild(details);
  }

  function openAuxInfo(id) {
    const info=auxInfoRegistry.get(id);if(!info)return;modalReturnFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;$('sourceModalTitle').textContent=info.title||'封國資料';$('sourceModalSummary').textContent=info.summary||'';const body=$('sourceModalBody');body.replaceChildren();
    if(info.kind==='local-officer'){
      renderLocalOfficerDetail(body,info);
  ...
  function closeModal(){
    sourceModal.hidden=true;document.body.classList.remove('modal-open');
  ...
  function chenTerritorySource(year) {
    ...
    return `史圖館${relation}矢量化（六點配準＋海岸校準）`;
  }
  ...
  function mapFeaturesWithSnapshotNames(features,snapshot) {
    const entities=new Map();
    for(const state of snapshot.states) {
      entities.set(state.id,state);
      for(const row of state.rows) {
        entities.set(row.id,row);
        for(const county of row.counties)entities.set(county.id,county);
      }
    }
    return (features||[]).map((feature)=>{
      const entity=entities.get(feature.entity_id);
      return entity ? {...feature,label:itemDisplayName(entity)} : feature;
    });
  }
  ...
  function selectMapLabels(labels,zoom=mapZoom) {
    ...
    for(const label of candidates) {
      ...
      points.get(pointKey).add(label.entityId||'');selected.push(label);
    }
    return selected;
  }
  ...
  function renderDynamicMap(year,snapshot,map) {
    ...
    const suppressed=new Set(fiefs.map((item)=>seatKey(item.level,item.x,item.y,item.sourceName)));
    ...
    for(const [level,kind,fontSize,priority,radius] of specs)for(const seat of seatsByLevel[level]) {
      ...
      if(!suppressed.has(seatKey(level,seat.x,seat.y,seat.n))) {
        ...
        if(linked.length)for(const feature of linked)labels[level].push({text:feature.label,x:seat.x,y:seat.y,fontSize,kind,priority,entityId:feature.entity_id});
          ...
          const areas=level==='state'?stateAreas:level==='prefecture'?prefAreas:[];
          const area=areas.find((item)=>item.k===seat.k&&snapshotFeatureForMapArea(item,level,[{level,x:seat.x,y:seat.y,entity:{name:seat.n}}]));
          labels[level].push({text:seat.n,x:seat.x,y:seat.y,fontSize,kind,priority,mapGroup:area&&areaGroups.get(area)});
        }
      }
    }
    ...
    for(const level of ['state','prefecture','county'])for(const label of labels[level]) {
      ...
      // Keep both geometries available; the zoom level selects one or both
      // label types without changing the annual entity or the original seat.
  ...
  function updateMethod() {
    const box=$('methodContent');box.replaceChildren();const paras=currentDynasty.key==='chen' ? [
      ...
      '本站州、郡、縣行政區劃採用各公元年最後時點的行政狀態，以解決同一年內行政區劃多次變化的展示問題。該「年末」原則僅適用於行政區劃，不適用於官員年表。地方長官部分收錄該年內能夠確認或推定曾任該職者，因此同一州、郡或縣在同一年中可能列有多任長官。',
    ...
    $('footerText').textContent=currentDynasty.key==='chen'
      ...
      : currentDynasty.key==='southern-liang'
        ? '資料依據：《中國行政區劃通史·三國兩晉南朝卷（下）》；縣郡人工複核工作簿；魯力《魏晉南北朝方鎮年表新編》；《梁書》《陳書》EPUB。首輪範圍、制度研究與待補資料見頁面連結。'
  ...
  function render() {
    ...
    const visible=renderResults(compared.states,compared.virtualFiefs,compared.extraGovernorStates,compared.detachedLocalOfficers,year,compared.pendingCountyAssignments), removed=filterRemoved(compared.removedChanges);
    ...
    $('statusText').textContent=currentDynasty.key==='chen'&&year===557
      ? `永定元年（557）只顯示史圖館矢量化疆域；現有《中國行政區劃通史》州郡縣資料從558年開始，故本年不反向補造行政區劃表。本年${localOfficerRecords(year).length}條地方長官記錄保留在「未連接記錄」區，不反向補造行政隸屬。`
      : baseline
  ...
  function switchDynasty() {
    ...
    $('fiefLegend').innerHTML=currentDynasty.key==='chen'
      ...
      : currentDynasty.key==='southern-liang'
        ? '<i class="legend-ruler">長沙王·某某／侯國·某某</i> 南朝封爵；郡級附爵者顯示為「某某國」；<i class="legend-ruler fief-uncertain-legend">斜體</i> 承襲存疑'
    ...
    if(localOfficerLegend){localOfficerLegend.hidden=!hasSouthernLayers;localOfficerLegend.innerHTML=currentDynasty.key==='southern-liang'
      ?'<i class="legend-local-officer">地方長官</i> 本年見載；〔授官〕履任未詳'
      :'<i class="legend-local-officer">地方長官</i> 本年曾任；<i class="legend-local-officer probable">斜體</i> 為推定';}
  ...
  document.addEventListener('click',(event)=>{
    ...
    const hotspot=event.target.closest('.map-hotspot,.map-label-link,.map-label-reference');if(hotspot){activateMapTarget(hotspot);return;}
  ...
  yearMapOverlay.addEventListener('keydown',(event)=>{if((event.key==='Enter'||event.key===' ')&&event.target.matches('.map-hotspot,.map-label-link,.map-label-reference')){event.preventDefault();activateMapTarget(event.target);}});
  ...
  yearMapViewport.addEventListener('pointerdown',(event)=>{
    if(event.button!==0||event.target.closest('.map-hotspot,.map-label-link,.map-label-reference'))return;
